...
| wildcard.resource.patterns | Sets the 'Resource patterns' compiler setting in IDEA which is useful both when compiling with IDEA (to specify which resources to copy) or with SBT (ensure that IDEA doesn't accidentally copy any resources by setting 'wildcard.resource.patterns=!*') | (as per IDEA) |
